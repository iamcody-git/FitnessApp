...
class userProfileController{
  public static async createUserProfile(req: AuthRequest, res: Response): Promise<void> {
    ...
    const fitnessLevelValue = fitness_level === "beginner" ? 1 : fitness_level === "intermediate" ? 2 : 3;
    const activityLevelValue = activity_level === "low" ? 1 : activity_level === "moderate" ? 2 : 3;
    const goalValue = goal === "weight_loss" ? 1 : goal === "strength" ? 2 : goal === "endurance" ? 3 : 4;
    ...
    const userProfileVector = [fitnessLevelValue, activityLevelValue, goalValue, genderValue];
    ...
    const recommendedPackageId = packageNameToIdMap[recommendedPackageNormalized];
